...
const confirmPurchaseOrder = async (req, res, next) => {
  try {
    ...
    // Notify requester and team of PO approval
    automationService.processPoApproval(order.id).catch(err =>
      console.error('[Automation] processPoApproval error:', err.message)
    );
