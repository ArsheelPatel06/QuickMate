...
export default function PurchaseOrderFormPage() {
  ...
  const fetchOrder = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api/v1'}/purchase-orders/${params.id}`, {
    ...
    const fetchMasterData = async () => {
      try {
        ...
        const [prodRes, vendorRes] = await Promise.all([
          ...
          fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api/v1'}/vendors?limit=1000`, { headers })
        ]);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api/v1'}/purchase-orders`, {
        method: 'POST',
        headers: { 
  ...
  const handleConfirm = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api/v1'}/purchase-orders/${order.id}/confirm`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await res.json();
  ...
  const handleReceive = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api/v1'}/purchase-orders/${order.id}/receive`, {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}` 
