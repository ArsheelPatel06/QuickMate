...
export default function ManufacturingOrderFormPage() {
  ...
  const fetchOrder = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api/v1'}/manufacturing-orders/${params.id}`, {
    ...
    const fetchMasterData = async () => {
      try {
        ...
        const [prodRes, bomsRes] = await Promise.all([
          ...
          fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api/v1'}/boms?limit=1000`, { headers })
        ]);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api/v1'}/manufacturing-orders`, {
        method: 'POST',
        headers: { 
  ...
  const handleCompleteWorkOrder = async (workOrderId: string) => {
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api/v1'}/manufacturing-orders/${order.id}/work-orders/${workOrderId}/complete`, {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}` 
