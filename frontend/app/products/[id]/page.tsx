...
export default function ProductFormPage() {
    ...
    if (!isNew) {
      const fetchProduct = async () => {
        try {
          ...
          const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api/v1'}/products/${params.id}`, {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const url = isNew 
        ? `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api/v1'}/products` 
        : `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api/v1'}/products/${params.id}`;
