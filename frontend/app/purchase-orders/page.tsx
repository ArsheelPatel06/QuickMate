...
export default function PurchaseOrderListPage() {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api/v1'}/purchase-orders?page=${page}&limit=10`, {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div>
          <Link
            ...
            className="inline-flex items-center gap-x-2 rounded-xl bg-indigo-700 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-indigo-800 transition-colors"
          >
                ...
                orders.map((order) => (
                  ...
                  >
                    ...
                    <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-900 text-right font-medium">
                      ₹{order.totalAmount ? parseFloat(order.totalAmount.toString()).toLocaleString('en-IN') : '0'}
                    </td>
