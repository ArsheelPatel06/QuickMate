...
export default function BomFormPage() {
    ...
    const fetchInitialData = async () => {
      try {
        ...
        const [prodRes, wcRes] = await Promise.all([
          ...
          fetch(`http://localhost:3000/api/v1/work-centers`, { headers })
        ]);
        ...
        if (!isNew) {
          const bomRes = await fetch(`http://localhost:3000/api/v1/boms/${params.id}`, { headers });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const url = isNew 
        ? 'http://localhost:3000/api/v1/boms' 
        : `http://localhost:3000/api/v1/boms/${params.id}`;
