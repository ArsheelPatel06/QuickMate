...
import { isConfigured, firebaseRegister } from '@/lib/firebase';
...
const ROLES = [
  ...
  { value: 'MANUFACTURING', label: 'Manufacturing Manager' },
...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isConfigured) {
        // ── Firebase Auth path ───────────────────────────────────────────────
        const { idToken, user } = await firebaseRegister(
      ...
      <div className="flex flex-1 flex-col items-center justify-center px-6 py-12">
        <div className="w-full max-w-sm">
          ...
          <p className="text-slate-500 mt-1 text-sm">
            ...
            <Link href="/login" className="font-semibold text-indigo-600 hover:text-indigo-800 transition-colors">Sign in</Link>
          </p>
          ...
          <form className="mt-8 space-y-4" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" required autoComplete="name" placeholder="Alex Johnson"
            ...
            <div>
              ...
              <select value={formData.role} onChange={update('role')}
                className="block w-full rounded-xl border border-slate-300 bg-white py-3 px-3 text-slate-900 text-sm focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-400/20 transition-all">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type={showPass ? 'text' : 'password'} required autoComplete="new-password"
                  placeholder="Min. 6 characters" value={formData.password} onChange={update('password')}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type={showConfirm ? 'text' : 'password'} required autoComplete="new-password"
                  placeholder="••••••••" value={formData.confirmPassword} onChange={update('confirmPassword')}
                  className="block w-full rounded-xl border border-slate-300 bg-white py-3 pl-11 pr-11 text-slate-900 text-sm placeholder:text-slate-400 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-400/20 transition-all" />
          ...
          <p className="mt-8 text-center text-xs text-slate-400">QuickMate ERP · Manufacturing Suite</p>
        </div>
      </div>
    </div>
  );
