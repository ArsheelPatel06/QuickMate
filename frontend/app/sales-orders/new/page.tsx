...
interface Product { id: string; name: string; sku: string; sellingPrice: string | number }
  ...
  const updateLine = (i: number, field: keyof Line, val: string | number) => {
    setLines(prev => {
      ...
      if (field === 'productId') {
        ...
        next[i] = { ...next[i], productId: val as string, unitPrice: p ? Number(p.sellingPrice) : 0 };
      } else {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        <Link href="/sales-orders" className="p-2 rounded-xl border border-slate-200 hover:bg-slate-50 text-slate-500 transition-colors">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Confirming will trigger automatic inventory check + MO creation</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-600 mb-1.5">Customer Name</label>
            <input
              type="text"
              required
              value={customerName}
              onChange={e => setCustomer(e.target.value)}
              placeholder="e.g. Priya Furniture Hub"
        ...
        <div className="flex gap-3">
          ...
          <button type="submit" disabled={submitting}
            ...
            {submitting ? <><Loader2 className="h-4 w-4 animate-spin" /> Creating…</> : 'Create Order (Draft)'}
          </button>
        </div>
      </form>
    </div>
