...
export default function SalesOrderFormPage() {
  ...
  const fetchOrder = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api/v1'}/sales-orders/${params.id}`, {
  ...
  useEffect(() => {
    const fetchMasterData = async () => {
      try {
        const token = localStorage.getItem('token');
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api/v1'}/products?limit=1000`, { 
          headers: { Authorization: `Bearer ${token}` } 
        });
        const data = await res.json();
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api/v1'}/sales-orders`, {
        method: 'POST',
        headers: { 
  ...
  const handleAction = async (endpoint: string) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api/v1'}/sales-orders/${order.id}/${endpoint}`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await res.json();
